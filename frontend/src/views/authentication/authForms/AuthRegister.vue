...
function validate() {
    const prom = Regform.value.validate();

    prom.then((res) => {
        
        if (res.valid) {
            // Form is valid, the api call can be made
            console.log('ON SUBMIT SUCCESS')
        } else {
            console.log('ON SUBMIT INVALID FORM')
        }

    }, (errpr) => {
        console.log('ON SUBMIT EROR', errpr)
    });
    ...
    <div>
        ...
        <v-form ref="Regform" lazy-validation action="/dashboards/analytical" class="mt-7 loginForm">
            <v-row>
                <v-col cols="12" sm="6">
                    <v-text-field v-model="firstname" density="comfortable" hide-details="auto" variant="outlined"
                        color="primary" label="Firstname">
                    </v-text-field>
                </v-col>
                <v-col cols="12" sm="6">
                    <v-text-field v-model="lastname" density="comfortable" hide-details="auto" variant="outlined"
                        color="primary" label="Lastname">
                    </v-text-field>
                </v-col>
            </v-row>
            <v-text-field v-model="email" type="email" :rules="emailRules" label="Email Address / Username" class="mt-4 mb-4" required
            ...
            <v-text-field v-model="password" :rules="passwordRules" label="Password" required density="comfortable"
                ...
                :type="show1 ? 'text' : 'password'" @click:append="show1 = !show1" class="pwdInput mb-4">
            </v-text-field>
            ...
            <div class="d-sm-inline-flex align-center mt-2 mb-7 mb-sm-0 font-weight-bold">
                <v-checkbox v-model="checkbox" :rules="[(v) => !!v || 'You must agree to continue!']"
                    label="Agree with?" required color="primary" class="ms-n2" hide-details>
                </v-checkbox>
